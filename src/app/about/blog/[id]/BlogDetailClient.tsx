...
export default function BlogDetailClient({ initialPost }: { initialPost: BlogPost }) {
  const params = useParams();
  const router = useRouter();
  const { t, i18n } = useTranslation();
  const postIdentifier = params?.id ? String(params.id) : null;

  const { data: post = initialPost } = useSWR<BlogPost | null>(
  ...
  useEffect(() => {
    if (post?.slug && postIdentifier && post.slug !== postIdentifier) {
      router.replace(`/about/blog/${post.slug}`);
    }
  }, [post?.slug, postIdentifier, router]);
  ...
  return (
    <div className="min-h-screen blog-paper mt-20 relative overflow-hidden">
      ...
      <article className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative z-10">
